...
def main() -> None:
    """Run corruption, re-evaluation, raw-source repair and comparison."""
    ...
    if not repaired_quality["success"]:
        raise RuntimeError("Repair quality gate failed; refusing to publish repaired index.")
    ...
    print("Metric comparison: baseline -> corrupted -> repaired")
    for metric in ("retrieval_hit_rate", "mean_token_f1", "judge_accuracy", "mean_judge_score"):
        print(
            f"{metric}: {baseline_metrics.get(metric, 0.0):.4f} -> "
            f"{corrupted_metrics_bundle.summary.get(metric, 0.0):.4f} -> "
            f"{repaired_metrics_bundle.summary.get(metric, 0.0):.4f}"
        )
